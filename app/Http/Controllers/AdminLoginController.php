...
class AdminLoginController extends Controller
{
    ...
    // die();
		...
		return view('Admin.admindashboard')->with(["order"=>$order]);
	}
      ...
      $a=DB::table('admin')->where('email',$email)->first();
        ...
        if(($email!="") && ($password!=""))
        {
          if($same>0 && Hash::check($password,$a->password))
          {
            ...
            return view('Admin.admindashboard')->with(['a'=>	$a,'order'=>$order]);
          }
          else
          {
  ...
  public function showads(Request $request)
  {
    ...
    return view('Admin.showpostitem')->with(['data'=>$data,'search'=>$search,'items'=>$items]);
  }
